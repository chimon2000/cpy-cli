...
test('ignore directories with flag "--nodir"', async t => {
	...
	fs.mkdirSync(path.join(t.context.tmp, 'nodir', 'nested'));
	fs.writeFileSync(path.join(t.context.tmp, 'nodir/nested/hello.js'), 'console.log("hello");');

	await execa('./cli.js', [path.join(t.context.tmp, 'nodir/**/*'), path.join(t.context.tmp, 'dest'), '--nodir']);
	t.is(read(t.context.tmp, 'nodir/nested/hello.js'), read(t.context.tmp, 'dest/hello.js'));
});
